...
$app->get('/mesajlar', function (Request $request, Response $response) {
    ...
    try{
        ...
        $mesajlar = $db->query("SELECT feed.feed_id, feed.feed,feed.created,users.username FROM feed inner join users on feed.user_id_fk=users.user_id")->fetchAll(PDO::FETCH_OBJ);
...
$app->get('/mesajlar/{id}', function (Request $request, Response $response) {
    ...
    try{
        $db = $db->connect();
        $mesajlar = $db->query("SELECT * FROM feed inner join users on users.user_id=feed.user_id_fk WHERE user_id_fk = $id")->fetchAll(PDO::FETCH_OBJ);
...
$app->post('/giris', function (Request $request, Response $response) {
    ...
    try{
        ...
        $db = $db->connect();
        $userData ='';
		$statement = "SELECT * FROM users WHERE (username=:email or email=:email) and password=:password";
		...
		$userData = $prepare->fetch(PDO::FETCH_OBJ);
            ...
            if($userData){
				$mail=internalUserDetails($userData->email);
            return $response
            ->withStatus(200)
            ->withHeader("Content-Type", 'application/json')
            //->withJson($user->token);
            ->withJson($mail, null, JSON_UNESCAPED_UNICODE | JSON_NUMERIC_CHECK);
        }
